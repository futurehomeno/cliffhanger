...
func (s *service) SendModeReport(_ bool) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("%s: failed to retrieve thermostat mode report: %w", s.Name(), err)
	}
	...
	if err != nil {
		return false, fmt.Errorf("%s: failed to send thremostat mode report: %w", s.Name(), err)
	}
...
func (s *service) SendSetpointReport(mode string, _ bool) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("%s: failed to retrieve thermostat setpoint report for mode %s: %w", s.Name(), normalizedMode, err)
	}
	...
	err = s.SendMessage(message)
	if err != nil {
		return false, fmt.Errorf("%s: failed to send thremostat state report: %w", s.Name(), err)
	}
...
func (s *service) SendStateReport(_ bool) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("%s: failed to retrieve thermostat state report: %w", s.Name(), err)
	}
