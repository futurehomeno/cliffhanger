...
func ResponseFromMessage(msg *fimpgo.FimpMessage) (*Response, error) {
	if msg.Type != EvtPD7Response {
		return nil, fmt.Errorf("response: incorrect message type, expected %s, got %s", EvtPD7Response, msg.Type)
	}
...
func NotifyFromMessage(msg *fimpgo.Message) (*Notify, error) {
	if msg.Payload.Type != EvtPD7Notify {
		return nil, fmt.Errorf("notify: incorrect message type, expected %s, got %s", EvtPD7Notify, msg.Payload.Type)
	}
